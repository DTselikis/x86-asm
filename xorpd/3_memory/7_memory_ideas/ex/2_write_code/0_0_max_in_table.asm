...
section '.data' data readable writeable
     my_tbl db     0x8f, 0x09, 0x2c, 0xca, 0x00, 0x35, 0xd3, 0xde, 0x6d, 0xed
      ...
      NUM_OF_ELEMENTS = ($ - my_tbl)

section '.text' code readable executable
start:
  xor   eax, eax     ; zero eax because we use only bl
  mov   al, byte [my_tbl + NUM_OF_ELEMENTS]   ; Load last element of the array
  mov   ecx, NUM_OF_ELEMENTS - 1    ; Loop until last-1 location (because above command)
find_max:
  mov   bl, byte[my_tbl + ecx]      ; Loop from end to start
  cmp   al, bl
  ja    next_itter    ; jg for signed comparison
  mov   al, bl
next_itter:
  loop  find_max
